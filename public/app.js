    ...
    <section class="auth">
      <form class="auth-panel" id="authForm">
        <div>
          ...
          <p>${mode === "login" ? "Log in to manage projects, owners, and deadlines." : "Create the first account as Admin, then invite members."}</p>
        </div>
        ${state.message}
  ...
  app.innerHTML = `
    <section class="shell">
      <aside class="sidebar">
        <div class="brand">
          <h1>Team Task Manager</h1>
          <p>${escapeHtml(state.user.name)} &middot; ${state.user.role}</p>
        </div>
        <nav class="nav">
